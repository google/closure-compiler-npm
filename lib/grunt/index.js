...
module.exports = function(grunt, extraArguments) {
  ...
  for (var key in extraArguments) {
    if (extraArguments.hasOwnProperty(key)) {
      gulpCompilerOptions[key] = extraArguments[key];
    }
  }
  ...
  function closureCompilerGruntTask() {
    ...
    closureCompilerGruntTask.updateOptions = function(opts) {
      for (var key in opts) {
        if (opts.hasOwnProperty(key)) {
          gulpCompilerOptions[key] = opts[key];
        }
      }
    };
